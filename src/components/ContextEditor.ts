...
export function ContextEditor(props: { onClose: () => void }): React.ReactElement {
  const { onClose } = props;
  ...
  // Load file on mount
  useEffect(() => {
    Deno.readTextFile(CONTEXT_PATH)
      .then((text) => {
        const ls = text.split("\n");
  ...
  useInput((input, key) => {
    // Save & close
    if (key.ctrl && input === "s") {
      Deno.writeTextFile(CONTEXT_PATH, state.lines.join("\n"))
        .then(() => onClose())
    ...
    // Line start/end (Ctrl+A / Ctrl+E, also Home/End via raw sequences)
  ...
  return h(
    ...
    h(
      ...
      h(Text, { bold: true, color: "green" },
        `✏  context.md${dirty ? "  [modified]" : ""}`),
      h(Text, { dimColor: true }, "[Ctrl+S] save  [Esc] discard"),
    ),
